      ...
      <div className="mb-4 rounded-2xl border border-brand-200/60 bg-white p-4 dark:border-brand-900/40 dark:bg-surface-dark">
        <form onSubmit={handleSearchSubmit} className="flex flex-wrap items-center gap-2">
          <div className="relative py-1 flex-1">
            {/* Icon */}
            <Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-brand-400 dark:text-white" size={20}  />
            <input
              type="text"
              value={searchInput}
              placeholder="Search Products ..."
              onChange={(e) => setSearchInput(e.target.value)}
              className="w-full pl-10 p-3 border border-gray-400 rounded-2xl font-semibold cursor-pointer dark:text-white dark:placeholder:text-white"
            />
          </div>{/* Search Input */}
