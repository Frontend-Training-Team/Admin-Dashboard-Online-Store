...
const StatCard = ({ icon: Icon, label, value, loading = false }) => (
  <div className="flex flex-col gap-3 rounded-2xl border border-brand-200/60 bg-white p-4 sm:p-5 shadow-xs dark:border-[rgba(255,255,255,0.06)] dark:bg-coal-800">
    ...
    <div>
      {loading ? (
        ...
        <p className="text-2xl sm:text-3xl font-bold text-brand-900 dark:text-content-primary">{value}</p>
      )}
      <p className="text-xs text-brand-500 dark:text-content-muted uppercase tracking-wider mt-1">{label}</p>
    </div>
  </div>
);
