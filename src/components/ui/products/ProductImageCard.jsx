...
function ProductImageCard({ src, index, onRemove, marked, displayStyle = "instant", onToggleMark, compact = false }) {
  if (displayStyle === "pill") {
    return (
      <div className={`relative rounded-lg overflow-hidden border border-gray-300 dark:border-[#2E364F] aspect-square`}>
        <img src={src} alt="Product" className="w-full h-full object-cover rounded-t-lg" />
  ...
  return (
    <div className={`flex flex-col gap-3 border border-gray-300 dark:border-[#2E364F] dark:bg-[#1E2435] rounded-lg ${compact ? "h-40" : "h-50"} ${marked ? "opacity-80" : ""}`}>
      <div className="relative">
        <img
          src={src}
          alt={`Product`}
          className={`w-full object-cover rounded-t-lg ${compact ? "h-30" : "h-40"}`}
        />
        ...
        {displayStyle === "overlay" && (
          <button
            ...
            className={`absolute top-1 right-1 rounded-full p-1.5 shadow ${marked ? "bg-red-500 text-white" : "bg-white text-gray-400"
              }`}
          >
            <Trash2 size={14} />
          </button>
