...
function ProductImageUploader({ images, onChange, markedKeys, onToggleMark, displayStyle, compact = false }) {
  ...
  return (
    <div className="flex flex-col gap-3 ">
      ...
      <label className={`w-full flex flex-col items-center gap-2 mt-2 border-2 text-[#8E4726] border-dashed border-[#8E4726BF] bg-[#FEE3C5] dark:bg-[#252C3F] dark:border-[#2E364F] rounded-lg cursor-pointer ${compact ? "p-2" : "p-6"}`}>
        <div className="flex flex-col items-center gap-1 p-2 text-[#8E4726]">
          <ImagePlus color="#8E4726" strokeWidth={1} className="w-6 h-6 mt-2 mb-1 dark:color-[#CCCCC] " />
          ...
          {images.map((img, i) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-center gap-1 text-xs border rounded-full px-3 py-1 ${marked ? "border-red-500 text-red-500 bg-red-500/10" : "border-gray-300 text-gray-400 dark:border-[#2E364F]"
                  }`}
              >
                <X size={12} />
                Remove
